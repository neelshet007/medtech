    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
        ...
        <div className="flex items-center gap-4">
          {session ? (
            <>
              ...
              <Link href={session.user?.role === "admin" ? "/admin/dashboard" : "/dashboard"} className="text-sm font-bold bg-slate-900 text-white px-5 py-2.5 rounded-full hover:bg-slate-800 transition-all">
                Dashboard
              </Link>
              <LogoutButton className="text-sm font-medium text-red-600 transition-colors hover:text-red-500">
                Log Out
              </LogoutButton>
            </>
          ) : (
      ...
      <main className="flex-grow flex flex-col items-center justify-center px-6 md:px-12 py-20 text-center">
        ...
        <h1 className="max-w-4xl text-5xl md:text-7xl font-extrabold text-slate-900 tracking-tight mb-6 leading-tight">
          ...
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-emerald-500">securely and instantly.</span>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
          ...
          <Link href="/dashboard" className="flex items-center justify-center gap-2 bg-white text-slate-700 border border-slate-200 px-8 py-4 rounded-full text-lg font-bold hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm">
            <Activity size={20} />
            Patient Dashboard
          </Link>
        </div>
      ...
      <section className="bg-white border-t border-slate-100 py-24 px-6 md:px-12">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <p className="text-slate-500 leading-relaxed">Razorpay integration with strict cryptographic signature verification ensures your transactions are tamper-resistant.</p>
          </div>
          ...
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <p className="text-slate-500 leading-relaxed">Monitor your vitals dynamically with interactive patient dashboard components.</p>
          </div>
          ...
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <p className="text-slate-500 leading-relaxed">Upload prescriptions and lab reports into a secure backend flow without exposing admin privileges.</p>
          </div>
        </div>
      </section>

      <footer className="bg-slate-900 text-slate-400 py-8 text-center text-sm border-t border-slate-800">
        <p>&copy; {new Date().getFullYear()} MediConnect E-Commerce. All rights reserved.</p>
      </footer>
    </div>
  );
}
