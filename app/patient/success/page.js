...
export default async function SuccessPage({ searchParams }) {
  ...
  const method = params.method || "razorpay";
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="bg-white p-8 md:p-12 rounded-2xl shadow-xl w-full max-w-md text-center border border-slate-100">
        ...
        <p className="text-slate-500 mb-6">
          {method === "cod"
            ? "Your cash on delivery order is confirmed and awaiting fulfillment."
            : "Your order has been confirmed successfully and is now being processed."}
        </p>
        ...
        <Link href="/dashboard/orders" className="block w-full py-3 px-4 bg-teal-600 text-white font-bold rounded-xl hover:bg-teal-700 transition">
          View Orders
        </Link>
      </div>
    </div>
